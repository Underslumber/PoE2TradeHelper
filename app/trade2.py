...
ITEM_BASE_MARKET_MIN_GENERAL_LOTS = 1
...
def _item_base_market_confirmed_lot_count(row: dict[str, Any]) -> int:
    counts: list[int] = []
    sample_lots = row.get("sample_lots") or []
    if sample_lots:
        counts.append(len(sample_lots))
    for group in row.get("price_currency_groups") or []:
...
def _item_base_market_row_has_evidence(row: dict[str, Any], min_lots: int = 1) -> bool:
...
def _item_base_market_min_visible_lots(q: str) -> int:
    return 1 if q.strip() else ITEM_BASE_MARKET_MIN_GENERAL_LOTS


def _normalize_item_base_market_price_trigger(value: Any) -> str:
    trigger = str(value or "").strip().lower()
    return trigger if trigger in {"above", "below"} else ""


def _item_base_market_row_target_price(row: dict[str, Any]) -> float | None:
    for key in ("low", "best", "median"):
        value = _to_float(row.get(key))
        if value is not None and value > 0:
            return value
    native = row.get("best_native")
    if isinstance(native, dict):
        value = _to_float(native.get("price_target"))
        if value is not None and value > 0:
            return value
    return None


async def _item_base_market_price_filter(
    *,
    league: str,
    target: str,
    price_trigger: str = "",
    ...
    trigger = _normalize_item_base_market_price_trigger(price_trigger)
    value = _to_float(price_value)
    if not trigger or value is None or value <= 0:
        return {"enabled": False}

    target_id = _currency_id(target) or str(target or "").strip()
    currency_id = _currency_id(price_currency) or target_id
    target_value = value
    error = ""
    if currency_id != target_id:
        _, rates = await _currency_rates_for_target(league, target, status="any")
        rate = _to_float(rates.get(currency_id) or rates.get(str(price_currency or "")))
        if rate is None or rate <= 0:
            target_value = None
            error = f"Нет сохраненного курса для {currency_id} -> {target_id}"
        else:
            target_value = value * rate
...
def _filter_item_base_market_rows_by_price(
    rows: list[dict[str, Any]],
    price_filter: dict[str, Any],
) -> list[dict[str, Any]]:
    if not price_filter.get("enabled"):
        return list(rows)
    threshold = _to_float(price_filter.get("target_value"))
    if threshold is None or threshold <= 0:
        return []
    trigger = _normalize_item_base_market_price_trigger(price_filter.get("trigger"))
    if not trigger:
        return list(rows)

    filtered = []
    for row in rows:
        price = _item_base_market_row_target_price(row)
        if price is None:
            continue
        if trigger == "below" and price <= threshold:
            filtered.append(row)
        elif trigger == "above" and price >= threshold:
            filtered.append(row)
    return filtered


def _apply_item_base_market_price_filter(
    payload: dict[str, Any],
    rows: list[dict[str, Any]],
    *,
    limit: int,
    ...
    rows = _filter_item_base_market_rows_by_price(rows, price_filter)
    payload["rows"] = rows[:limit]
    payload["matched_total"] = len(rows)
    payload["priced_total"] = sum(1 for row in rows if _item_base_market_row_target_price(row) is not None)
    if price_filter.get("enabled"):
        payload["price_filter"] = price_filter
    else:
        payload.pop("price_filter", None)
    return payload
...
def _item_base_market_rows_matching_min_ilvl(
    ...
    matching_rows = []
    for row in rows:
        sample_lots = [lot for lot in row.get("sample_lots") or [] if isinstance(lot, dict)]
        lots_with_level = [lot for lot in sample_lots if _to_int(lot.get("item_level")) is not None]
        if lots_with_level:
            filtered_lots = [lot for lot in lots_with_level if (_to_int(lot.get("item_level")) or 0) >= min_ilvl]
            ...
            filtered_row = {
                ...
                **_base_market_stats(
                    filtered_lots,
                    _to_int(row.get("fetched_count")) or _to_int(row.get("raw_count")) or len(sample_lots),
                    _to_int(row.get("stale_count")) or 0,
                ),
                "sample_lots": _base_market_sample_lots(filtered_lots),
                "min_ilvl": min_ilvl,
            }
...
async def get_item_base_market(
    ...
    cache_key = _item_base_market_cache_key(league, target, status, q, limit, collection_min_ilvl)
    ...
    if not force_refresh:
        ...
        if job and time.time() - float(job.get("created_ts") or 0) < ITEM_BASE_MARKET_JOB_TTL:
            ...
            pending = _item_base_market_pending_result(
                league=league,
                target=target,
                status=status,
                q=q,
        ...
        rows = _sort_item_base_market_rows(_base_market_catalog_rows(catalog, min_ilvl=collection_min_ilvl))
        payload = _item_base_market_exact_result(
            league=league,
            target=target,
            status=status,
            q=q,
