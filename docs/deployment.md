...
Для кнопки пробуждения компьютера добавь в серверный `.env` его MAC-адрес и параметры домашней сети:

```dotenv
WOL_TARGET_MAC=
WOL_TARGET_IP=192.168.1.2
WOL_BROADCAST_ADDRESS=192.168.1.255
WOL_AUTO_DISCOVER=true
```

Если `WOL_TARGET_MAC` пуст, сервис ищет MAC адреса `192.168.1.2` через локальную таблицу соседей (`ip neigh`/ARP) и сохраняет его в `DATA_DIR/wake_on_lan_mac.json`. Для первого обнаружения компьютер должен быть включён и виден серверу хотя бы один раз. Сервис должен работать в той же локальной сети и иметь право отправлять UDP broadcast. Доступ к кнопке есть только у администратора приложения; учётные данные роутера не используются.
